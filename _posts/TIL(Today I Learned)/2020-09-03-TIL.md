...
## Carving Secure Wi-Fi Zones with Defensive Jamming 논문 읽기
### 3. `JAM WHERE IT HURTS`
First, we briefly introduce the IEEE802.15.4 PHY frame reception and identify the points of action to intercept it.  
However, it is important to note that the overall concept presented in this work is not limited to either a specific medium access protocol (MAC) or physical layer (PHY).  
